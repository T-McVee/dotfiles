...
4. Derive a conventional commit PR title from commit history
...
For each commit (or logical group), identify *what changed* from a user or system perspective — not implementation details:
...
Synthesize a [conventional commit](https://www.conventionalcommits.org/) title: `<type>(<scope>): <short description>`
...
**Scope**: optional but useful. Derive from the branch name or the most-changed area — e.g., `auth`, `api`, `dashboard`. Omit if there's no clear scope.
