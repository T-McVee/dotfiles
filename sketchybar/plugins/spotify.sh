...
update ()
{
  ...
  if [ "$(echo "$INFO" | jq -r '.["Player State"]')" = "Playing" ]; then
    ...
    TRACK_NAME="$(echo "$INFO" | jq -r .Name)"
    if [ ${#TRACK_NAME} -gt $CHAR_LIMIT ]; then
        TRACK="$(echo "$TRACK_NAME" | cut -c1-$CHAR_LIMIT)..."
    else
        TRACK="$TRACK_NAME"
    fi

    # TRACK="$(echo "$INFO" | jq -r .Name | cut -c1-$CHAR_LIMIT)"
    ARTIST="$(echo "$INFO" | jq -r .Artist | cut -c1-$CHAR_LIMIT)"
